...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="CSV file to log forward-only cp/org MSE per step (default: <output_dir>/rl_study_loss_log.csv)",
    )
    ...
    print(f"ORG dataset: {args.org_image}")
    ...
    if args.study_log_file is None:
        args.study_log_file = os.path.join(args.output_dir, "rl_study_loss_log.csv")
    ...
    for step in progress_bar:
        ...
        # Study-only forward losses (no backward, no optimizer update).
        with torch.inference_mode():
            eval_cp_batch = next(cp_loader)
            (
                eval_cp_noisy_latents,
                eval_cp_timesteps,
                eval_cp_noise,
                eval_cp_prompt_embeds,
                eval_cp_pooled_prompt_embeds,
                eval_cp_add_time_ids,
                eval_cp_sigma_mask,
            ) = encode_batch_for_rl(
                eval_cp_batch,
                vae,
            ...
            for name in wr_names:
                ...
                if lora1_module is not None:
                    tlora_delta = compute_orthogonal_lora_weight_delta(
                        lora1_module,
                        mask=eval_cp_sigma_mask,
                    )
                    eval_tlora_deltas[name] = tlora_delta.to(
                        device=w0_weights[name].device,
                        dtype=w0_weights[name].dtype,
                    )

            eval_overrides = build_unet_wr_delta_overrides(
                unet_override_specs=unet_override_specs,
                wr_weights=wr_weights,
                deltas=eval_tlora_deltas,
            )

            eval_cp_pred = _module_functional_call(
                unet,
                eval_overrides,
                eval_cp_noisy_latents,
                eval_cp_timesteps,
                encoder_hidden_states=eval_cp_prompt_embeds,
                added_cond_kwargs={
                    "text_embeds": eval_cp_pooled_prompt_embeds,
                    "time_ids": eval_cp_add_time_ids,
                },
                cross_attention_kwargs={"sigma_mask": eval_cp_sigma_mask},
            ).sample
            eval_cp_loss = F.mse_loss(eval_cp_pred.float(), eval_cp_noise.float(), reduction="mean")

            eval_org_batch = next(org_loader)
            (
                eval_org_noisy_latents,
                eval_org_timesteps,
                eval_org_noise,
                eval_org_prompt_embeds,
                eval_org_pooled_prompt_embeds,
                eval_org_add_time_ids,
                eval_org_sigma_mask,
            ) = encode_batch_for_rl(
                eval_org_batch,
                vae,
                ...
                alpha_rank_scale,
                max_timestep,
                timesteps=eval_cp_timesteps.detach(),
                noise=eval_cp_noise.detach(),
            )

            eval_org_pred = _module_functional_call(
                unet,
                eval_overrides,
                eval_org_noisy_latents,
                eval_org_timesteps,
                encoder_hidden_states=eval_org_prompt_embeds,
                added_cond_kwargs={
                    "text_embeds": eval_org_pooled_prompt_embeds,
                    "time_ids": eval_org_add_time_ids,
                },
                cross_attention_kwargs={"sigma_mask": eval_org_sigma_mask},
            ).sample
            eval_org_loss = F.mse_loss(eval_org_pred.float(), eval_org_noise.float(), reduction="mean")

        cp_loss_val = float(eval_cp_loss.item())
        org_loss_val = float(eval_org_loss.item())
        log_writer.writerow(
            {
                "step": step,
        ...
        progress_bar.set_postfix(
            {
                ...
                "mse": f"{avg_mse:.6f}",
                "cp_fwd": f"{cp_loss_val:.6f}",
                "org_fwd": f"{org_loss_val:.6f}",
