...
class CopyrightDreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        print(
            f"  - {self.num_regularization_samples} regularization samples (1-f={1-f})"
        )
    ...
    def _generate_all_samples(self):
        ...
        for _ in tqdm(
            range(self.num_copyright_samples), desc="Generating copyright samples"
        ):
            ...
            if idx < 20:
                ...
                image_filename = os.path.join(
                    example_dir, f"sample_{idx:02d}_copyright.png"
                )
            ...
            # Process image
            image = generated_image.resize(
                (self.size, self.size), resample=Image.BICUBIC
            )
        ...
        for _ in tqdm(
            range(self.num_regularization_samples),
            desc="Generating regularization samples",
        ):
            ...
            if idx < 20:
                # Save image
                image_filename = os.path.join(
                    example_dir, f"sample_{idx:02d}_regularization.png"
                )
                generated_image.save(image_filename)
...
def main():
    parser = argparse.ArgumentParser(
        description="DreamBooth LoRA training for SDXL with Copyright Protection"
    )
    ...
    trainable_params_text1 = sum(
        p.numel() for p in text_encoder.parameters() if p.requires_grad
    )
    trainable_params_text2 = sum(
        p.numel() for p in text_encoder_2.parameters() if p.requires_grad
    )
    total_trainable = (
        trainable_params_unet + trainable_params_text1 + trainable_params_text2
    )
    ...
    print(
        f"  UNet trainable parameters: {trainable_params_unet:,} / {total_params_unet:,}"
    )
    print(
        f"  Text Encoder 1 trainable parameters: {trainable_params_text1:,} / {total_params_text1:,}"
    )
    print(
        f"  Text Encoder 2 trainable parameters: {trainable_params_text2:,} / {total_params_text2:,}"
    )
